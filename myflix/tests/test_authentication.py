...
class AuthenticationUserTestCase(APITestCase):
    ...
    def test_autenticacao_de_user_username_incorreto(self):
        """Teste que verifica uma requisição GET de usuario com username incorreto"""
        user = authenticate(username='c3pp',password='123456')
        self.assertFalse((user is not None) and user.is_authenticated)

    def test_autenticacao_de_user_password_incorreto(self):
        """Teste que verifica uma requisição GET de usuario com password incorreto"""
        user = authenticate(username='c3po',password='123457')
        self.assertFalse((user is not None) and user.is_authenticated)
